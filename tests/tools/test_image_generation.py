...
class TestRegistryIntegration:
    ...
    def test_schema_exposes_prompt_aspect_ratio_and_optional_reference_image(self, image_tool):
        """The agent-facing schema stays tight: model/provider selection is
        config-level, while an optional local reference image is allowed for
        providers that support image-conditioned generation."""
        props = image_tool.IMAGE_GENERATE_SCHEMA["parameters"]["properties"]
        assert set(props.keys()) == {"prompt", "aspect_ratio", "image_url"}
        assert "image_url" not in image_tool.IMAGE_GENERATE_SCHEMA["parameters"].get("required", [])
